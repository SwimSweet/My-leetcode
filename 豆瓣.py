...
def getmessage(html):
    ...
    for movie_list in movie_lists.find_all('li'):
      try:
        ...
        movie_num=movie_pics.find('em').get_text()
        print(movie_num)
        movie_info=movie_list.find('div',attrs={'class':'info'})
        movie_hd=movie_info.find('div',attrs={'class':'hd'})
        movie_name=movie_hd.find('span',attrs={'class':'title'}).get_text()
        ...
        for item in message_list:
            ...
            if len(keys)<=1:
                continue
            mes[keys[0]]=keys[1]
        movie_director=mes.get('导演',"")
        movie_actor=mes.get('主演',"")
        movie_type=mes.get('类型',"")
        movie_area=mes.get('制片国家/地区',"")
        movie_language=mes.get('语言',"")
        ...
        movie_short_show=movie_soup.find('span',attrs={'property':'v:summary'}).get_text().split('……')[0].strip().replace(' ','').replace('\n', '')
